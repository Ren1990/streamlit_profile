...
st.set_page_config(page_title="Working Experience", page_icon="💼", layout="wide")
margin_r,body,margin_l = st.columns([0.1, 3, 0.1])

with body:
    menu()
    st.header("💼 Working Experience",divider='rainbow')
    st.write("")
    col1,  col2 = st.columns([3,2])
    with col1:
        st.subheader(":blue[Sr. Business Analyst | Micron Semiconductor Asia]")
    with col2:
    ...
    col1,  col2 = st.columns([3,2])
    with col1:
        st.subheader(":blue[Sr. Process Engineer | Micron Semiconductor Asia]")
    with col2:
